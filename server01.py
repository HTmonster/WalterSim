...
servers=[
   #名称       id   是否本服务器    地址
   ["server00", 0,  False, "http://127.0.0.0.1:5000"],
   ["server01", 1,  True,"http://127.0.0.0.1:5001"]
]
...
def propagate(x):
   ...
   for server in servers:
      ...
      if not server[2]:
         serverUrl=server[3]
         res=requests.post("http://127.0.0.0.1:5001/propagate",data={"x":x,"id":siteID})
   ...
   for server in servers:
      ...
      if not server[2]:
         serverUrl=server[3]
         res=requests.post(serverUrl+"/ds_durable,",data={"x":x,"id":siteID})
           #TODO:f+1 情况 和返回信息确认 先空缺
   # 标志
